...
import { CheckCircle, AlertCircle, TrendingUp, Stethoscope, Save } from 'lucide-react'
import './App.css'
...
function App() {
  ...
  const saveToGoogleSheets = async (data) => {
    try {
      ...
      // Como é no-cors, assumimos sucesso se não houver erro
      setSaveSuccess(true)
  ...
  const handleSubmit = async () => {
    ...
    const dataToSave = {
      ...
      pontuacao: score,
      classificacao: result.level,
      percentual: Math.round((score / 25) * 100),
      respostas: checkedItems
    }

    console.log('Dados do diagnóstico:', dataToSave)
